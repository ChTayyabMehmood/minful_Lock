...
class FgMethodCallHandler(
    ...
    private val turkeyModeServiceConn =
        SafeServiceConnection(
            context = context,
            serviceClass = TurkeyModeService::class.java
        )

    // Broadcast receiver to clear restriction whitelist on ADB unlock
    private val turkeyModeStopReceiver = object : BroadcastReceiver() {
        override fun onReceive(ctx: Context, intent: Intent) {
            if (intent.action == AppConstants.ACTION_STOP_TURKEY_MODE) {
                if (trackerServiceConn.isActive) {
                    trackerServiceConn.service?.getRestrictionManager?.updateTurkeyModeApps(null)
                }
            }
        }
    }
    ...
    init {
        ...
        turkeyModeServiceConn.bindService()

        // Register receiver to clear restriction whitelist on ADB unlock
        context.registerReceiver(
            turkeyModeStopReceiver,
            IntentFilter(AppConstants.ACTION_STOP_TURKEY_MODE)
        )
    }
    ...
    fun dispose() {
        ...
        turkeyModeServiceConn.unBindService()

        // Unregister broadcast receiver
        try {
            context.unregisterReceiver(turkeyModeStopReceiver)
        } catch (_: Exception) { }
    }
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            ...
            "updateTurkeyModeApps" -> {
                val turkeyApps = JsonUtils.parseStringSet(call.arguments() ?: "")
                // Update tracker service restriction manager
                if (trackerServiceConn.isActive) {
                    trackerServiceConn.service?.getRestrictionManager?.updateTurkeyModeApps(turkeyApps)
                } else if (turkeyApps.isNotEmpty()) {
                    trackerServiceConn.setOnConnectedCallback { service ->
                        service.getRestrictionManager.updateTurkeyModeApps(turkeyApps)
                    }
            ...
            "startTurkeyModeService" -> {
                val args = call.arguments<Map<String, Any>>() ?: mapOf()
                val apps = args["whitelistedApps"] as? List<*> ?: listOf<String>()
                val isSession = args["isSessionBased"] as? Boolean ?: false
                val grayscale = args["enableGrayscale"] as? Boolean ?: true
                val durationSec = args["sessionDurationSec"] as? Int ?: 0

                val intent = Intent(context, TurkeyModeService::class.java).apply {
                    action = TurkeyModeService.ACTION_START_TURKEY
                    putStringArrayListExtra(
                        TurkeyModeService.EXTRA_WHITELISTED_APPS,
                        ArrayList(apps.map { it.toString() })
                    )
                    putExtra(TurkeyModeService.EXTRA_IS_SESSION_BASED, isSession)
                    putExtra(TurkeyModeService.EXTRA_ENABLE_GRAYSCALE, grayscale)
                    putExtra(TurkeyModeService.EXTRA_SESSION_DURATION_SEC, durationSec)
                }
                context.startForegroundService(intent)
                result.success(true)
            }

            "stopTurkeyModeService" -> {
                // Stop the turkey mode service
                val intent = Intent(context, TurkeyModeService::class.java).apply {
                    action = TurkeyModeService.ACTION_STOP_TURKEY
                }
                context.startService(intent)

                // Clear turkey mode from tracker
                if (trackerServiceConn.isActive) {
                    trackerServiceConn.service?.getRestrictionManager?.updateTurkeyModeApps(null)
                }
                result.success(true)
            ...
            "toggleGrayscale" -> {
                val enable = call.arguments<Boolean>() ?: false
                result.success(GrayscaleManager.setEnabled(context, enable))
            }

            "hasWriteSecureSettings" -> {
                result.success(GrayscaleManager.hasPermission(context))
            }

            "setUnlockPassphraseHash" -> {
                val passphrase = call.arguments<String>() ?: ""
                val hash = if (passphrase.isNotEmpty()) {
                ...
                SharedPrefsHelper.getSetUnlockPassphraseHash(context, hash)
                result.success(hash)
            }

            "storePassphraseHashDirectly" -> {
                val hash = call.arguments<String>() ?: ""
                SharedPrefsHelper.getSetUnlockPassphraseHash(context, hash)
                result.success(true)
            }

            "isTurkeyModeServiceRunning" -> {
                result.success(
