...
object SharedPrefsHelper {
    ...
    private const val PREFS_UNLOCK_PASSPHRASE_HASH = "unlockPassphraseHash"
    ...
    /**
     * Fetches the stored unlock passphrase hash if hash is null, else stores it.
     */
    fun getSetUnlockPassphraseHash(context: Context, hash: String?): String {
        checkAndInitializeUniquePrefs(context)
        hash?.let {
            mUniquePrefs!!.edit().putString(PREFS_UNLOCK_PASSPHRASE_HASH, it).apply()
            return it
        }
        return mUniquePrefs!!.getString(PREFS_UNLOCK_PASSPHRASE_HASH, "") ?: ""
    }
