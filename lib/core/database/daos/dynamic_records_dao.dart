...
import 'package:mindful/core/database/tables/turkey_mode_sessions_table.dart';
...
class DynamicRecordsDao extends DatabaseAccessor<AppDatabase>
  ...
  // ==================================================================================================================
  // ===================================== TURKEY MODE SESSIONS ========================================================
  // ==================================================================================================================

  /// Fetch the last active [TurkeyModeSession] from database, if not found then return null
  Future<TurkeyModeSession?> fetchLastActiveTurkeyModeSession() async =>
      (select(turkeyModeSessionsTable)
            ..where((e) => e.endDateTime.equals(DateTime(0)))
            ..limit(1))
          .getSingleOrNull();

  /// Inserts a single [TurkeyModeSession] object in the database.
  Future<TurkeyModeSession> insertTurkeyModeSession({
    required bool isSessionBased,
    required List<String> whitelistedApps,
  }) async =>
      into(turkeyModeSessionsTable).insertReturning(
        TurkeyModeSessionsTableCompanion.insert(
          isSessionBased: Value(isSessionBased),
          startDateTime: Value(DateTime.now()),
          whitelistedApps: Value(whitelistedApps),
        ),
        mode: InsertMode.insertOrReplace,
      );

  /// Updates a single [TurkeyModeSession] record by primary key
  Future<void> updateTurkeyModeSessionById(TurkeyModeSession session) async =>
      update(turkeyModeSessionsTable).replace(session);

  /// Loads the number of turkey mode sessions
  Future<int> fetchTurkeySessionsCount() async {
    final query = selectOnly(turkeyModeSessionsTable)
      ..addColumns([turkeyModeSessionsTable.id.count()]);

    return await query
        .map((row) => row.read(turkeyModeSessionsTable.id.count()) ?? 0)
        .getSingle();
  }

  /// Loads the total duration for all turkey mode sessions
  Future<Duration> fetchLifetimeTurkeySessionsDuration() async {
    final totalDurationQuery = selectOnly(turkeyModeSessionsTable)
      ..where(turkeyModeSessionsTable.endDateTime.isNotValue(DateTime(0)))
      ..addColumns([turkeyModeSessionsTable.durationSecs.sum()]);

    final totalDuration = await totalDurationQuery
        .map((row) => row.read(turkeyModeSessionsTable.durationSecs.sum()) ?? 0)
        .getSingle();
