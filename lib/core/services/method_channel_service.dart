...
class MethodChannelService {
  ...
  /// Updates the whitelisted apps for turkey mode in the tracker service.
  Future<void> updateTurkeyModeApps(List<String> whitelistedApps) async =>
      _methodChannel.invokeMethod(
        'updateTurkeyModeApps',
        jsonEncode(whitelistedApps),
      );

  /// Starts the turkey mode foreground service with the given configuration.
  Future<void> startTurkeyModeService({
    required List<String> whitelistedApps,
    required bool isSessionBased,
    required bool enableGrayscale,
  ...
  /// Stops the turkey mode foreground service.
  Future<void> stopTurkeyModeService() async =>
      await _methodChannel.invokeMethod('stopTurkeyModeService');

  /// Toggles system grayscale mode.
  Future<bool> toggleGrayscale(bool enable) async =>
      await _methodChannel.invokeMethod('toggleGrayscale', enable);

  /// Checks if the app has WRITE_SECURE_SETTINGS permission for grayscale.
  Future<bool> hasWriteSecureSettings() async =>
      await _methodChannel.invokeMethod('hasWriteSecureSettings');

  /// Computes and stores SHA-256 hash of the passphrase on the native side.
  /// Returns the hex hash string.
  Future<String> setUnlockPassphraseHash(String passphrase) async =>
      await _methodChannel.invokeMethod('setUnlockPassphraseHash', passphrase);

  /// Stores an already-computed passphrase hash directly to native SharedPreferences.
  Future<void> storePassphraseHashDirectly(String hash) async =>
      await _methodChannel.invokeMethod('storePassphraseHashDirectly', hash);

  /// Checks whether the TurkeyModeService is currently running on the native side.
  Future<bool> isTurkeyModeServiceRunning() async =>
      await _methodChannel.invokeMethod('isTurkeyModeServiceRunning');
