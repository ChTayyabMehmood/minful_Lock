...
class RestrictionManager(
    ...
    // Turkey Mode (whitelist-based: only these apps are allowed)
    private var turkeyModeApps = setOf<String>()
    private var isTurkeyModeActive = false
    ...
    fun updateTurkeyModeApps(apps: Set<String>?) {
        turkeyModeApps = apps ?: emptySet()
        isTurkeyModeActive = apps != null && apps.isNotEmpty()
        if (apps == null) stopIfNoUsage.invoke()
        Log.d(TAG, "updateTurkeyModeApps: Turkey mode apps updated: $turkeyModeApps, active: $isTurkeyModeActive")
    }


    // returns nearest time stamp for rechecking
    fun isAppRestricted(packageName: String): RestrictionState? {
        // TURKEY MODE: Highest priority - block if not in whitelist
        if (isTurkeyModeActive && packageName !in turkeyModeApps) {
            return RestrictionState(type = RestrictionType.TURKEY_COLD_BLOCK)
        }
