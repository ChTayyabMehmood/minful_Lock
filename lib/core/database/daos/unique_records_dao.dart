...
import 'package:mindful/core/database/tables/turkey_mode_table.dart';
...
class UniqueRecordsDao extends DatabaseAccessor<AppDatabase>
  ...
  /// Saves a single [TurkeyMode] object to the database.
  Future<void> saveTurkeyMode(TurkeyMode turkeyMode) async =>
      into(turkeyModeTable).insert(turkeyMode, mode: InsertMode.insertOrReplace);

  /// Loads the first (and likely only) [TurkeyMode] object
  /// from the database. If none exists, returns default instance.
  Future<TurkeyMode> loadTurkeyMode() async =>
      await select(turkeyModeTable).getSingleOrNull() ?? defaultTurkeyModeModel;
}
